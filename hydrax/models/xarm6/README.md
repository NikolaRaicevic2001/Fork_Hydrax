...
MPPI push-T task (see `hydrax/tasks/pusht.py`'s `robot="xarm6"` option and
`hydrax/models/xarm6_pusht_clutter/`).

**Status: wired into `hydrax`.** This directory used to be a staging area
(`Fork_Hydrax/robot_assets/xarm6/`, kept outside `hydrax/` to avoid colliding
...
`LICENSE`) already matched the convention used by `hydrax/models/pusht_clutter/`
and `hydrax/models/g1/`, so the move needed no further restructuring.
...
- `xarm6.xml` -- the robot model: 5 actuated joints, meshes, materials,
  ...
  in, e.g. `hydrax/models/xarm6_pusht_clutter/`).
...
- `verify_model.py` -- standalone load/sanity-check/render script, no
  hydrax imports. Re-run after any edit to `xarm6.xml`.
...
7. **Added: the pushing stick.** Not present in the source at all (its
   ...
   used by `hydrax/models/pusht/pusht.xml`).
...
`verify_model.py` (run standalone, no hydrax): loads `scene.xml` with no
...
- No integration into a `hydrax.Task`/`ConsensusTask` -- this is pure
