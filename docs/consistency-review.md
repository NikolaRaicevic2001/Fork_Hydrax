...
| ~~2.3~~ | ~~Variance annealing~~ | off | off | **Resolved (revised during task 5's tuning pass):** first made 2D match 3D's `on, residual-driven` default; then, tuning 3D against a real 600-step run, found this makes divergence *worse* -- the primal residual never converges for this task, so `noise = clip(kappa*residual, min, max)` just pins near `noise_max` permanently (measured: final pos error 4.65 with it on vs. 2.0 with it off, identical seed/config otherwise). Turned off on both sides instead, which is what 1.4's own analysis already predicted was the risk with this signal. |
